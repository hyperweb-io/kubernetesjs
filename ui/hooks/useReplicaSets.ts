...
export function useScaleReplicaSet() {
  ...
  return {
    ...
    mutate: ({ name, replicas, namespace }) =>
      mutation.mutate({ path: { namespace: namespace || defaultNamespace, name }, query: {}, body: { spec: { replicas } } }),
    mutateAsync: ({ name, replicas, namespace }) =>
      mutation.mutateAsync({ path: { namespace: namespace || defaultNamespace, name }, query: {}, body: { spec: { replicas } } }),
  }
}
