...
async function deleteResources(options: any): Promise<void> {
  ...
  if (configType === 'cluster') {
    ...
    console.log(chalk.blue('You can set up a new cluster using: interweb setup -c ' + configPath));
  } else {
    console.log(chalk.blue('\nThe application has been completely removed.'));
    console.log(chalk.blue('You can redeploy the application using: interweb deploy -c ' + configPath));
  }
}
