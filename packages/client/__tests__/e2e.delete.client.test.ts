...
describe('SetupClient deleteOperators: basic flow with config', () => {
  ...
  it('runs deleteOperators on config', async () => {
    ...
    // After deletion, it's valid for status to be 'pending' (operators not installed)
    expect(status.phase === 'ready' || status.phase === 'installing' || status.phase === 'pending').toBe(true);
  });
});
