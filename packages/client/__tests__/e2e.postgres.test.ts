...
describe('Postgres deploy (CloudNativePG) end-to-end', () => {
  ...
  const client = new Client({ restEndpoint: K8S_API, namespace: 'default', verbose: true });
  ...
  it('installs CNPG operator and deploys Postgres (basic readiness)', async () => {
    ...
    const op: OperatorConfig = { name: 'cloudnative-pg', version: CNPG_VERSION, enabled: true, namespace: CNPG_NAMESPACE } as OperatorConfig;
    ...
    const ep: any = await api.readCoreV1NamespacedEndpoints({ path: { namespace: dbNs, name: `${clusterName}-rw` }, query: {} as any });
    const subsets = ep?.subsets || [];
    const readyCount = subsets.reduce((acc: number, s: any) => acc + ((s.addresses || []).length || 0), 0);
    expect(readyCount).toBeGreaterThan(0);

    // Verify pooler pods are Ready (typed CoreV1 API)
    const pods: any = await api.listCoreV1NamespacedPod({ path: { namespace: dbNs }, query: { labelSelector: `cnpg.io/poolerName=${poolerName}` } as any });
    const items: any[] = pods?.items || [];
    expect(items.length).toBeGreaterThan(0);
    const allReady = items.every(p => (p.status?.containerStatuses || []).every((c: any) => c.ready));
