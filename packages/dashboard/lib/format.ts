...
import { Lesson } from '@/data/courses/types';
import { downloads, LabelText, ProductDownloadKey } from '@/data/products';
...
const formatDownloadNumber = (number: number): string => {
	const million = 1_000_000;

	const formatter = new Intl.NumberFormat('en', {
		notation: 'compact',
		compactDisplay: 'short',
		maximumFractionDigits: number < million ? 0 : 1,
	});

	return formatter.format(number);
};

export const formatProductLabel = (labelText: LabelText) => {
	if (labelText.type === 'custom') return labelText.customText;

	const downloadCount = downloads[labelText.packageName as ProductDownloadKey];

	if (!downloadCount) {
		throw new Error(`No download count found for product: ${labelText.packageName}`);
	}

	return `${formatDownloadNumber(downloadCount)}+ Monthly Downloads`;
};

export const getLessonThumbnail = (lesson: Lesson) => {
	const { video, markdown } = lesson;
	const isVideoType = video !== undefined;

	if (isVideoType) {
		const { videoId, customThumbnail } = video;
		return customThumbnail ? trimPath(customThumbnail) : getYoutubeThumbnail(videoId);
	}

	if (!markdown) {
		throw new Error('Markdown content is required for non-video lessons');
	}

	return trimPath(markdown.headerImage);
};
