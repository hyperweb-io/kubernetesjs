...
export const useHyperwebAssets = () => {
	...
	return useQuery({
		...
		queryFn: async () => {
			if (!rpcEndpoint || !address || !prices) return null;
			...
			const assets: PrettyAsset[] = balances
				...
				.map(({ amount, denom }) => {
					...
					const dollarValue = calcDollarValue(amount, asset, prices);
			...
			return {
				assets,
				totalDollarValue: calcTotalDollarValue(assets),
			};
		},
		enabled: !!rpcEndpoint && !!address && !!prices,
	});
};
